...
PARTITION (dt, hour, minute, seconds)
select split(StartTime, '[ ]')[1], Dur, Proto, SrcAddr, Dir, DstAddr, Dport, State, sTos, dTos, TotPkts, TotBytes, Label, split(starttime, '[ ]')[0], split(split(starttime, '[ ]')[1], '[:]')[0], split(split(starttime, '[ ]')[1], '[:]')[1], split(split(split(starttime, '[ ]')[1], '[:]')[2], '[.]')[0] from staged_rand_netflow;
