...
insert into table public_netflow_botnet_44_nocomp
PARTITION (dt, hour, minute)
select split(StartTime, '[ ]')[1], Dur, Proto, SrcAddr, Sport, Dir, DstAddr, Dport, State, sTos, dTos, TotPkts, TotBytes, SrcBytes, Label, split(starttime, '[ ]')[0], split(split(starttime, '[ ]')[1], '[:]')[0], split(split(starttime, '[ ]')[1], '[:]')[1] from staged_netflow_ctu_botnet_44;
