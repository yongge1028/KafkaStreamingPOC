...
insert into table rand_netflow_snappy
PARTITION (dt, hour, minute)
select split(StartTime, '[ ]')[1], Dur, Proto, SrcAddr, Dir, DstAddr, Dport, State, sTos, dTos, TotPkts, TotBytes, Label, Country, split(starttime, '[ ]')[0], split(split(starttime, '[ ]')[1], '[:]')[0], split(split(starttime, '[ ]')[1], '[:]')[1] from staged_rand_netflow;
