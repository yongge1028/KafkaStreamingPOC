...
class SimpleLibContext(config: Config) {
  ...
  config.checkValid(ConfigFactory.defaultReference(), "netflow-lib")
...
//  val foo = config.getString("netflow-lib.foo")
//  val hello = config.getString("netflow-lib.hello")
//  val whatever = config.getString("netflow-lib.whatever")
}
